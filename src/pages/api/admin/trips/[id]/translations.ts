...
import { listTranslationsForTrip, upsertTripTranslation } from '../../../../../lib/i18n-content';
...
export const GET: APIRoute = async ({ request, params }) => {
	...
	const trip = await getTripById(env.DB, id);
	if (!trip) return Response.json({ error: 'Not found' }, { status: 404 });
	const translations = await listTranslationsForTrip(env.DB, id);
	return Response.json({ trip, translations });
};
...
export const PUT: APIRoute = async ({ request, params }) => {
	...
	await upsertTripTranslation(env.DB, id, locale, {
